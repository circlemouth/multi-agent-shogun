...
const run = async () => {
  ...
  try {
    ...
    summary.evidence.screenshots.push(
      await writeLocatorShot(page.locator('#charts-actionbar .charts-actions__controls'), '00-action-controls'),
    );
    ...
    if (sendDisabled === false) {
      ...
      if (guardVisible) {
        ...
        summary.evidence.screenshots.push(
          await writeLocatorShot(actionBar.locator('.charts-actions__controls'), '05-action-controls-after'),
        );
      }
    }
  } catch (error) {
