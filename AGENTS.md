...
multi-agent-shogun は「母艦（キュー/ダッシュボード/通知）」であり、特定プロジェクトの実作業・成果物の正本ではない。

- 正本は各プロジェクトのリポジトリ/作業ディレクトリに置く（`projects/<id>.yaml` の `path` 等で外部パスを指す）
- このリポジトリ内に、別プロジェクトの `docs/` / `artifacts/` / `scripts/` / `server/` などを複製しない
- もし誤ってコピーが作られても、**このリポジトリ側のコピーは正本とみなさない**

## セッション開始時の必須行動（全エージェント必須）

新たなセッションを開始した際（初回起動時）は、作業前に必ず以下を実行せよ。
※ これはコンパクション復帰とは異なる。セッション開始 = Codexを新規に立ち上げた時の手順である。
...
2. **自分の役割に対応する instructions を読め**:
   - 将軍 → instructions/codex-shogun.md
   - 家老 → instructions/codex-karo.md
   - 足軽 → instructions/codex-ashigaru.md
...
> - **セッション開始**: Codexの新規起動。白紙の状態からMemory MCPでコンテキストを復元する
...
この手順は AGENTS.md（自動読み込み）のみで完結する。instructions/codex-ashigaru.md は初回復帰時には読まなくてよい（2タスク目以降で必要なら読む）。
...
### /clear復帰の禁止事項
- instructions/codex-ashigaru.md を読む必要はない（コスト節約。2タスク目以降で必要なら読む）
...
Layer 4: Session（揮発・コンテキスト内）
  └─ AGENTS.md（自動読み込み）, instructions/codex-*.md
...
| Layer 4: Session | instructions/codex-shogun.md | instructions/codex-karo.md | instructions/codex-ashigaru.md |
...
- **WriteやEditの前に必ずReadせよ。** Codex/Claude Codeは未読ファイルへのWrite/Editを拒否する。Read→Write/Edit を1セットとして実行すること。
...
## 指示書
- instructions/codex-shogun.md - 将軍の指示書
- instructions/codex-karo.md - 家老の指示書
- instructions/codex-ashigaru.md - 足軽の指示書
